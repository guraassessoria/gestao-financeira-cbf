...
(async()=>{
 ...
 const [{data:m},{data:contas},{data:lanc}] = await Promise.all([
   sb.from('de_para_dre').select('codigo_conta_contabil,codigo_centro_custo').eq('codigo_linha_dre','1704').single(),
 ...
 const matchConta=(lanc,mapa)=>{const a=n(lanc), b=n(mapa); if(!a||!b) return false; if(a===b) return true; if(!(classeBy.get(b)||'').toLowerCase().startsWith('sint')) return false; const vis=new Set(); let cur=a; while(cur&&!vis.has(cur)){vis.add(cur); const s=supBy.get(cur); if(!s) break; if(s===b) return true; cur=s;} return false;}
 ...
 const cc=n(m.codigo_centro_custo), conta=m.codigo_conta_contabil;
 let totalEnt=0,totalCc=0,totalEither=0;
 for(const r of lanc||[]){const v=Number(r.valor||0); let dEnt=0,cEnt=0,dCc=0,cCc=0;
   if(matchConta(r.cta_debito,conta)){
     if(n(r.ocorren_deb)===cc) dEnt+=v;
     if(n(r.c_custo_deb)===cc) dCc+=v;
   }
   if(matchConta(r.cta_credito,conta)){
     if(n(r.ocorren_crd)===cc) cEnt+=v;
     if(n(r.c_custo_crd)===cc) cCc+=v;
   }
   const sign = natureza(conta)==='dev' ? 1 : -1;
   totalEnt += sign*(dEnt-cEnt);
   totalCc += sign*(dCc-cCc);
   totalEither += sign*((dEnt||dCc? v:0) - (cEnt||cCc? v:0));
 }
 console.log(JSON.stringify({linha:'1704',conta,cc,totalByEntidade:totalEnt,totalByCentroCusto:totalCc,totalEither},null,2));
})();
