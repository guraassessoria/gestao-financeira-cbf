...
async function fetchAllLancamentos(sb, periodo) {
  const PAGE = 1000;
  const { count } = await sb.from('lancamentos_contabeis').select('*', { count: 'exact', head: true }).eq('periodo', periodo);
  console.log(`Total lancamentos for ${periodo}:`, count);
  const all = [];
  for (let page = 0; page * PAGE < count; page++) {
    const { data } = await sb.from('lancamentos_contabeis')
...
(async () => {
  ...
  const condBy = new Map((contas || []).map(c => [comp(c.cod_conta), c.cond_normal || 'Credora']));
  const ocorrByCC = new Map((ccs || []).filter(c => c.ocorrencia).map(c => [norm(c.cod_cc), comp(c.ocorrencia)]));

  console.log('CCs loaded:', (ccs || []).length, '| with ocorrencia:', ocorrByCC.size);

  const lanc = await fetchAllLancamentos(sb, periodo);
  ...
  for (const m of dePara || []) {
    ...
    console.log(`${m.codigo_linha_dre} | ${m.codigo_conta_contabil} | ocorren=${mapOcorren} | ${formatted} (${matched} entries matched)`);
  }
})();
