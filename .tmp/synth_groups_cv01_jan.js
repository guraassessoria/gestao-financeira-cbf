...
(async()=>{
 ...
 const [{data:ct1},{data:lanc}] = await Promise.all([
   sb.from('contas_contabeis').select('cod_conta,cta_superior,classe').or('cod_conta.like.321%,cta_superior.like.321%').limit(50000),
 ...
 function calc(group){let deb=0,cred=0; for(const r of lanc||[]){const v=Number(r.valor||0); if(match(r.cta_debito,group)&&n(r.ocorren_deb)==='1') deb+=v; if(match(r.cta_credito,group)&&n(r.ocorren_crd)==='1') cred+=v;} return deb-cred;}
 console.log(JSON.stringify({g321101:calc('321101'),g3211:calc('3211'),g321:calc('321')},null,2));
})();
