...
(async()=>{
 ...
 const {data}=await sb.from('contas_contabeis').select('cod_conta,descricao,classe,cta_superior').like('cod_conta','321301%').order('cod_conta');
 console.log('321301*');
 console.log(JSON.stringify(data,null,2));
 const {data:data2}=await sb.from('contas_contabeis').select('cod_conta,descricao,classe,cta_superior').like('cod_conta','321502%').order('cod_conta');
 console.log('321502*');
